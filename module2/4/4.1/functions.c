...
int addContact(PhoneBook *phone_book) {
  ...
  if (phone_book->count > 1){
    new_contact.prev = &phone_book->contacts[phone_book->count - 1];
    phone_book->contacts[phone_book->count - 1].next = &new_contact;
  }
  phone_book->count++;
  ...
  for (int j = 0; j < phone_book->count; j++) {
    if (phone_book->contacts[j].id == contact_id) {
      ...
      if (j > 1 && j < phone_book->count - 1){
        phone_book->contacts[j-1].next = &phone_book->contacts[j+1];
      phone_book->contacts[j+1].prev = &phone_book->contacts[j-1];
      }
      else if (j <= 1) phone_book->contacts[j+1].prev = NULL;
      else if (j >= phone_book->count - 1) phone_book->contacts[j-1].next = NULL;
